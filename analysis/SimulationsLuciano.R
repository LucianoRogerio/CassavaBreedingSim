...
#install_github("wolfemd/AlphaSimHlpR", force = T)
...
start <- proc.time()[3]
postBurnIn_NVDP1_IITA_GS <- runSchemesPostBurnIn(simulations = burnIn_IITA_PS,
                                      newBSP = bsp2, # so you can change the scheme after burn-in
                                      nPostBurnInCycles = 12,
                                      selCritPop = "parentSelCritGEBV",
                                      selCritPipe = "productSelCritBLUP",
...
saveRDS(postBurnIn_NDVP1_IITA_GS,file = here::here("output","postBurnIn_NVDP1_IITA_GS.rds"))
rm(postBurnIn_NDVP1_IITA_GS)
