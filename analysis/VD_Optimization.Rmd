...
meanGplot1 <- forSimPlot %>% filter(YearPostBurnIn >= -3) %>%
              group_by(BS, ParentSel, YearPostBurnIn, year) %>% 
              ...
              geom_ribbon(aes(ymin = meanGenMean - seGenMean, 
                              ...
                              fill = BS, alpha = ParentSel)) + 
              geom_line(aes(y = meanGenMean, color = BS, linetype = ParentSel)) + 
              theme_minimal()
...
sdGplot1 <- forSimPlot %>% filter(YearPostBurnIn >= -3) %>%
            ...
            geom_ribbon(aes(ymin = meanGenSD - seGenSD, 
                            ...
                            fill = BS, alpha = ParentSel)) + 
            geom_line(aes(y = meanGenSD, color = BS, linetype = ParentSel)) + 
            theme_minimal()
...
meanGplot2 <- forSimPlot %>% filter(YearPostBurnIn >= -3) %>%
              ...
              geom_smooth(method = "loess", alpha = 0.25,
                          aes(color = BS, linetype = ParentSel)) + 
              theme_minimal()

sdGplot2 <- forSimPlot %>% filter(YearPostBurnIn >= -3) %>%
            group_by(BS, ParentSel, YearPostBurnIn, year, stage) %>% 
