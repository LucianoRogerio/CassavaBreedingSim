...
```{r Figure 1 Candidates IITA Breeding Scheme 1 to 3 Yrs, fig.width=10, echo = FALSE, message = FALSE, warning = FALSE}
...
forSimPlot <- readRDS(here::here("output", "DataSimPlotIITA1_3_Par.rds")) %>%
  ...
  mutate(CandPop = factor(ifelse(YearAsCand == 1, "CET",
                             ...
                             levels = c("CET", "CET-PYT",
                                        "CET-AYT", "AYT-UYT2",
                                        "AYT-UYT2B")),
         SelMethod = factor(ifelse(CandPop %in% c("AYT-UYT2", "AYT-UYT2B"), "REML/BLUP", "GBLUP"))) %>%
...
meanGplot1 <- forSimPlot %>% 
              ...
              theme_minimal() + labs(y = expression(italic(TGV)),
                                     x = "Year")
...
Accuracyplot2 <- forSimPlot %>%
  ...
  labs(y = expression(italic(hat("r")["GEBV,TBV"])),
       x = "Year")
...
forSimPlot <- readRDS(here::here("output", "DataSimPlotIITA1_3_Acc.rds")) %>%
  ...
  mutate(CandPop = factor(ifelse(YearAsCand == 1, "CET",
                             ...
                             levels = c("CET", "CET-PYT",
                                        ...
                                        "AYT-UYT2B")),
         SelMethod = factor(ifelse(CandPop %in% c("AYT-UYT2", "AYT-UYT2B"), "REML/BLUP", "GBLUP")),
         accProgSel = accClSel[,1],
         accVDPSel = accAtSel[,1],
         cycle = cycle.y, .keep = "unused") %>% 
  select(SimRep, CandPop, SelMethod, YearPostBurnIn, year, cycle, accProgSel, accVDPSel) %>% 
  reshape2::melt(data = ., id.vars = c("SimRep", "CandPop", "SelMethod", "YearPostBurnIn", "year", "cycle"),
                 variable.name = "Pred", value.name = "Acc")

forSimPlot2 <- forSimPlot %>% filter(!(CandPop == "AYT-UYT2" & Pred == "accProgSel"),
                                     !(CandPop == "AYT-UYT2B" & Pred == "accProgSel"),
                                     !(CandPop == "CET" & Pred == "accProgSel" & YearPostBurnIn < 0),
                                     !(CandPop == "CET-PYT" & Pred == "accProgSel" & YearPostBurnIn < 0),
                                     !(CandPop == "CET-AYT" & Pred == "accProgSel" & YearPostBurnIn < 0),
                                     !(CandPop == "CET" & Pred == "accVDPSel" & YearPostBurnIn >= 0),
                                     !(CandPop == "CET-PYT" & Pred == "accVDPSel" & YearPostBurnIn >= 0),
                                     !(CandPop == "CET-AYT" & Pred == "accVDPSel" & YearPostBurnIn >= 0))

Accuracyplot2 <- forSimPlot2 %>%
  group_by(CandPop, SelMethod, YearPostBurnIn, year, Pred) %>%
  summarise(meanAcc = mean(Acc, na.rm = T),
            sdAcc = sd(Acc, na.rm = T) / sqrt(n())) %>%
  ggplot(., aes(x = YearPostBurnIn, color = CandPop, linetype = SelMethod)) +
  geom_ribbon(aes(ymin = meanAcc - sdAcc, 
                  ymax = meanAcc + sdAcc,
                  fill = CandPop), alpha = 0.25) + 
  geom_line(aes(y = meanAcc)) + ggtitle("Clone Selection Accuracy at CET") + 
  theme_minimal() + theme(legend.position="bottom") + ylim(0,0.75) +
  labs(y = expression(italic(hat("r")["GEBV,TGV"])),
       x = "Year")

#tiff(filename = here::here("output", "Figures", "AccuracyCET.tiff"),
#units = "cm", compression = "lzw", width = 20, height = 10, res = 250)
Accuracyplot2
#dev.off()
