...
### IITA breeding Scheme 1º alternative (Four years VDP)
```{r message=FALSE, warning=FALSE}
library(tidyverse); library(reactable)
...
saveRDS(burnIn_IITA_PS,file = here::here("output","BurnIn_IITA.rds"))
rm(start); rm(end)

source(here::here("code","runSchemesPostBurnIn.R"))
burnIn_IITA_PS <- readRDS(here::here("output", "BurnIn_IITA.rds"))
start <- proc.time()[3]
...
##### Graph with the genetic gain for  Stage

```{r, eval = F}
postBurnInPS <- readRDS(here::here("output", "postBurnIn_IITA_PS.rds"))
postBurnInPS_NDVP1 <- readRDS(here::here("output", "postBurnIn_NVDP1_IITA_PS.rds"))
...
forSimPlot <- forSimPlot[!(forSimPlot$PostBurnIn == "IITA" & forSimPlot$stage == "UYT1"),]
forSimPlot <- forSimPlot[!(forSimPlot$year <= 25 & forSimPlot$stage == "UYT1"),]
...
#              seGenMean = sd(genValMean) / n()) %>% 
              ggplot(., aes(x = YearPostBurnIn, y = genValMean, colour = PostBurnIn)) +
...
#                      seGenSD = sd(genValSD) / n()) %>% 
            ggplot(., aes(x = YearPostBurnIn, y = genValSD, colour = PostBurnIn)) +
            geom_smooth(method = "loess", alpha = 0.5)# +
...
tiff(filename = here::here("output", "SimResults.tiff"), units = "cm", compression = "lzw",
     res = 350, height = 10, width = 20)
...
### Simulations with the IITA breeding Scheme 1º alternative (Four years VDP)
