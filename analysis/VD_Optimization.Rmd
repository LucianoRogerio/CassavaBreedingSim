...
forSimPlot <- readRDS(here::here("output", "DataSimPlotIITA1_3_Par.rds")) %>%
  ...
  mutate(YearAsCand = factor(ifelse(YearAsCand == 1, "CET",
...
BoxPlotPar <- forSimPlot %>% 
  ...
  mutate(YearAsCand = as.character(YearAsCand)) %>%
  group_by(SimRep, YearAsCand,YearPostBurnIn) %>%
  slice_max(gebv, n = 100) %>%
...
##### Genomic Prediction Accuracy for the CET
...
forSimPlot <- readRDS(here::here("output", "DataSimPlotIITA1_3_Acc.rds")) %>%
  ...
  select(SimRep, YearAsCand, YearPostBurnIn, year, cycle, accProgSel, accVDPSel) %>% 
  reshape2::melt(data = ., id.vars = c("SimRep", "YearAsCand", "YearPostBurnIn", "year", "cycle"),
                 variable.name = "Pred", value.name = "Acc")

Accuracyplot2 <- forSimPlot %>%
  group_by(YearAsCand, YearPostBurnIn, year, Pred) %>%
  summarise(meanAcc = mean(Acc, na.rm = T),
            sdAcc = sd(Acc, na.rm = T) / sqrt(n())) %>%
  ggplot(., aes(x = YearPostBurnIn, color = YearAsCand)) +
  geom_ribbon(aes(ymin = meanAcc - sdAcc, 
                  ymax = meanAcc + sdAcc,
                  fill = YearAsCand, linetype = Pred), alpha = 0.25) + 
  geom_line(aes(y = meanAcc, linetype = Pred)) + ggtitle("Clone Selection Accuracy at CET") + 
  theme_minimal() + theme(legend.position="bottom")
