...
forSimPlot <- readRDS(here::here("output", "DataSimPlotIITA1_3_Par.rds")) %>%
  ...
  mutate(YearAsCand = ifelse(YearAsCand == 1, "CET",
                             ifelse(YearAsCand == 2, "CET-PYT",
                                    ifelse(YearAsCand == 3, "CET-AYT", "AYT-UYT2"))),
         YearAsCand = factor(YearAsCand, levels = c("CET", "CET-PYT",
                                                    "CET-AYT", "AYT-UYT2"))) %>%
...
BoxPlotPar <- forSimPlot %>% mutate(YearPostBurnIn = Year - 20) %>%
  filter(YearPostBurnIn >= -3, YearAsCand != "PS") %>%
