...
IITA1Yr <- readRDS(here::here("output", "postBurnIn_IITA1yr_GS.rds"))
IITA2Yr <- readRDS(here::here("output", "postBurnIn_IITA_GS.rds"))
IITA3Yr <- readRDS(here::here("output", "postBurnIn_IITA3yr_GS.rds"))
...
forSimPlotIITA_1_3_UYT <- IITA1Yr %>%
  mutate(YearAsCand = 1) %>%
  bind_rows(IITA2Yr %>% 
              mutate(PYearAsCand = 2)) %>%
  bind_rows(IITA3Yr %>% 
              mutate(PYearAsCand = 3)) %>%
  unnest_wider(SimOutput) %>%
  select(SimRep, YearAsCand, records) %>% 
  unnest_wider(records) %>%
  select(SimRep, YearAsCand, stageOutputs) %>%
