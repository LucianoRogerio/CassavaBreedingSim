...
IITAPS  <- readRDS(here::here("postBurnIn_IITA_PS.rds"))
IITA1Yr <- readRDS(here::here("postBurnIn_IITA_GS_1Yr.rds"))
IITA2Yr <- readRDS(here::here("postBurnIn_IITA_GS_2Yrs.rds"))
IITA3Yr <- readRDS(here::here("postBurnIn_IITA_GS_3Yrs.rds"))
...
forSimPlotIITA_Acc <- IITAPS %>%
  ...
  select(SimRep, YearAsCand, Year, cycle, grmSize, accAtSel) %>%
  mutate(YearPostBurnIn = Year - 20)

ParIITAPS <- read.table(here::here("ParentOriginPS.csv"), header = T, sep = ",")
ParIITA1Yr <- read.table(here::here("ParentOriginGS_1Yr.csv"), header = T, sep = ",")
ParIITA2Yr <- read.table(here::here("ParentOriginGS_2Yrs.csv"), header = T, sep = ",")
ParIITA3Yr <- read.table(here::here("ParentOriginGS_3Yrs.csv"), header = T, sep = ",") 
...
forSimPlotIITA_Par <- IITAPS %>%
  ...
  unnest(grmData) %>%
  mutate(YearPostBurnIn = Year - 20) %>%
  filter(pop == "c", YearPostBurnIn >=-3)
forSimPlotIITA_Par$stage <- schemeDF$stageNames[forSimPlotIITA_Par$stage]

saveRDS(forSimPlotIITA_Acc, file = here::here("DataSimPlotIITA1_3_Acc.rds"))
